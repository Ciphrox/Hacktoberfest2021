...
class Kruskal_Algorithm {  
 ...
 Kruskal_Algorithm(int vertices, int edges) {  //to create a graph  
 ...
 void applyKruskal() {  
     ...
     Edge finalResult[] = new Edge[vertices];  //to store the final MST  
     int newEdge = 0;  
     int index = 0;  
     for (index = 0; index < vertices; ++index)  
         finalResult[index] = new Edge();  

     Arrays.sort(edgeArray);  //to sort the edges  
 ...
 void performUnion(Subset subsetArray[], int sourceRoot, int destinationRoot) {  //to perform union of two sets 
 ...
 public static void main(String[] args) {  
     ...
     Kruskal_Algorithm graph = new Kruskal_Algorithm(v, e); // to create Graph  
